...
const int Column_Nums=23045;//矩阵列数 
const int Non_Zero_Eliminator_Nums=18748;//非零消元子行数  
const int Eliminated_Line_Nums=14325;//被消元行行数 
//int Eliminated_Bit_Size = Column_Nums / 32 + 1;//每个实例中的byte型数组数 
...
public:
    ...
    {
        FirstOne=-1;//no value
        //(int *)_aligned_malloc(1024, 1024 * sizeof(int));     
        Eliminated_Bytes=(int *)aligned_alloc(1024, 1024 * sizeof(int));
        for(int i=0;i<Eliminated_Bit_Size;i++)
        {
            Eliminated_Bytes[i]=0;
