...
int main(){
	...
	for(int k=0;k<N;k++)
	{
        __m128 vt= _mm_set_ps(m[k][k],m[k][k],m[k][k],m[k][k]);
        if(k%4!=0)//align
        {
            for(int j=k;j<(k+4)/4*4;j++)
            {
                m[k][j]=m[k][j]/m[k][k];
            }
        }
		...
		for(int i=k+1;i<N;i++)
		{
			...
			for(int j=k;j<N;j+=4)
			{
				...
				else//pal
				{
					...
					__m128 temp3= _mm_set_ps(m[i][k],m[i][k],m[i][k],m[i][k]);
