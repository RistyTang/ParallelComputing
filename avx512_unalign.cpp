...
public:
    ...
    Eliminator_Bitmap()
    {
        ...
        Eliminated_Bytes = (int *)aligned_alloc(64, Eliminated_Bit_Size * sizeof(__m512i));
    ...
    {  
        int i=0;
    for (; i+16 <= Eliminated_Bit_Size; i+=16)
    {
        __m512i byte1 = _mm512_load_epi32(Eliminated_Bytes + i);
        __m512i byte2 = _mm512_load_epi32(b.Eliminated_Bytes + i);
        byte1 = _mm512_xor_si512(byte1, byte2);
        _mm512_store_epi32(Eliminated_Bytes + i, byte1);
    }
    ...
    {
        unsigned int mask = _mm512_movepi32_mask(*(__m512i*)(Eliminated_Bytes+i));
        if (mask != 0)
        {
            FirstOne = i * 32 + __builtin_ctz(mask);
            return;
        }
    }
    }
